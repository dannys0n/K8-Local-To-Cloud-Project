...
Invoke-Kubectl apply -k "$Root/infra/observability"
...
Invoke-Kubectl rollout status deployment/kube-state-metrics -n kube-system --timeout=180s
Invoke-Kubectl rollout status deployment/grafana -n tcp-lab --timeout=180s
